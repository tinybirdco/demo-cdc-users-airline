...
USERS_TABLE_NAME = 'users'  # e.g. users. Should be lowercase to avoid issues with case sensitivity in different databases, but you can name it something else if you want.
...
MYSQL_DB_NAME = 'mysql_cdc_demo'  # e.g. mysql_cdc_demo. Again note the use of _.
MYSQL_USERNAME = 'admin'  # e.g. admin
MYSQL_PASSWORD = ''  # e.g. MySecretPassword!

# Confluent Cloud Cluster connection details
# These details are used to connect to your Confluent Cloud Kafka Cluster to manage the Topic.
...
CONFLUENT_UNAME = ''  # This is your Confluent Identity from your Auth details for the cluster. Not to be confused with your Confluent Cloud API key.
...
# These details are used to create the Kafka Connectors in Confluent Cloud. They are not the same as the Confluent Cloud Cluster connection details.
...
TINYBIRD_CONFLUENT_CONNECTION_NAME = CONFLUENT_CLUSTER_NAME  # Friendly name to use in Tinybird for the Confluent Cloud connection. It defaults to the Confluent Cluster name.